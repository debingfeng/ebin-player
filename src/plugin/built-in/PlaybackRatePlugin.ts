...
class PlaybackRatePluginImpl extends BasePlugin<RateConfig, RateExports> {
  ...
  validateConfig = (config: unknown) => {
    ...
    const validOptions =
      ...
      c.options.every(
        (o) => typeof o?.value === "number" && typeof o?.label === "string",
      );
    ...
    return {
      ...
      errors: valid
        ? []
        : [
            "配置不合法: defaultRate 必须为 number, options 必须为 {value,label}[]",
          ],
    };
  };
  ...
  async onInit(ctx: PluginContext) {
    ...
    // 添加到主控制栏而不是独立悬浮
    const controlBar = this.ctx.player
      .getContainer()
      .querySelector(".ebin-player-control-bar");
    if (controlBar) {
    ...
    const setRate = (rate: number) => {
      const allowed = this.ctx
        .getConfig<RateConfig>()
        .options.some((o) => o.value === rate);
...
export const PlaybackRatePlugin: PluginDefinition<RateConfig, RateExports> =
  new PlaybackRatePluginImpl();
