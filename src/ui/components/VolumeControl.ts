...
export class VolumeControl extends BaseComponent {
  ...
  protected setupEventListeners(): void {
    ...
    if (this.volumeButton) {
      ...
      this.addEventListener(
        this.volumeContainer as HTMLElement,
        "mouseleave",
        () => {
          this.hideSlider();
        },
    ...
    if (this.volumeSlider) {
      ...
      this.addEventListener(this.volumeSlider, "mouseenter", () =>
        this.showSlider(),
      );
      ...
      // 视口变化时重新计算位置
      this.addEventListener(window as unknown as HTMLElement, "resize", () =>
        this.adjustSliderPosition(),
      );
      this.addEventListener(window as unknown as HTMLElement, "scroll", () =>
        this.adjustSliderPosition(),
      );
    }
  ...
  private toggleMute(): void {
    try {
      ...
      if (!isMuted && currentVolume > 0) {
        ...
        this.logger.debug("Muted, volume set to 0", {
  ...
  private startDrag(e: MouseEvent | Touch): void {
    ...
    this.logger.debug("Volume drag started", {
      startY: this.dragStartY,
      startVolume: this.dragStartVolume,
    });
  }
  ...
  private handleDrag(e: MouseEvent | Touch): void {
    ...
    const sliderHeight =
      (this.config as VolumeControlConfig).sliderHeight || 100;
    const volumeChange = deltaY / sliderHeight; // 根据滑块高度计算音量变化
    const newVolume = Math.max(
      0,
      Math.min(1, this.dragStartVolume + volumeChange),
    );
  ...
  private adjustSliderPosition(): void {
    if (!this.volumeSlider || !this.volumeButton || !this.volumeContainer)
      return;
  ...
  private handleKeyboardControl(e: KeyboardEvent): void {
    ...
    this.logger.debug("Keyboard volume change", {
      from: currentVolume,
      to: newVolume,
    });
    this.player.setVolume(newVolume);
    ...
    if (this.volumeButton) {
      const icon =
        state.muted || state.volume === 0
          ? "🔇"
          : this.getVolumeIcon(state.volume);
  ...
  protected applyTheme(): void {
    ...
    this.volumeButton.addEventListener("focus", () => {
      this.volumeButton!.style.outline =
