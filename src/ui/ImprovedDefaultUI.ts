 ...
 * 基于组件化架构，解决原有问题
 */
import { PlayerInstance, PlayerState, ControlBarConfig, PlayerTheme, Logger } from '../types';
...
export class ImprovedDefaultUI {
  ...
  constructor(
    ...
    if (playerLogger && typeof playerLogger === 'object' && 'setEnabled' in playerLogger) {
      ...
      this.logger = {
        ...
        debug: (...args: any[]) => console.debug('UI:ImprovedDefault:', ...args),
        info: (...args: any[]) => console.info('UI:ImprovedDefault:', ...args),
        warn: (...args: any[]) => console.warn('UI:ImprovedDefault:', ...args),
        error: (...args: any[]) => console.error('UI:ImprovedDefault:', ...args),
      };
    }
   ...
   * 转换旧版配置到新版配置格式
   */
  private convertLegacyConfig(config: ControlBarConfig, theme: PlayerTheme): Partial<UIConfig> {
    return {
      enabled: true,
      theme: 'default',
      controlBar: {
  ...
  updateTheme(theme: Partial<PlayerTheme>): void {
    ...
    // 更新主题管理器
    if (theme.primaryColor || theme.secondaryColor || theme.backgroundColor || theme.textColor) {
      // 创建自定义主题
      const customTheme = {
        id: 'custom',
        name: '自定义主题',
