...
export function logMethod(options: LogMethodOptions = {}) {
  const { includeArgs = true, includeResult = false, time = true, level = 'debug', message } = options;
  return function (_target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    const original = descriptor.value;
    descriptor.value = function (...args: any[]) {
      const logger = (this as any).logger as { [k: string]: (...a: any[]) => void } | undefined;
      const enabled = !!logger && typeof logger[level] === 'function';
      const label = message || propertyKey;
      const start = time ? (typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now()) : 0;
      ...
      try {
        ...
        if (result && typeof result.then === 'function') {
          return (result as Promise<any>)
            .then((res) => {
              if (enabled) {
                const duration = time ? ((typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now()) - start) : undefined;
            ...
            .catch((err) => {
              if (logger && typeof logger.error === 'function') {
                const duration = time ? ((typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now()) - start) : undefined;
                logger.error(`${label} rejected`, { args: includeArgs ? args : undefined, duration, error: err });
              }
              throw err;
            });
        }
        ...
        if (logger && typeof logger.error === 'function') {
          const duration = time ? ((typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now()) - start) : undefined;
          logger.error(`${label} threw`, { args: includeArgs ? args : undefined, duration, error: err });
        }
        throw err;
      }
    };
