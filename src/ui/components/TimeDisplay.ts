...
export class TimeDisplay extends BaseComponent {
  ...
  private cycleFormat(): void {
    const config = this.config as TimeDisplayConfig;
    const formats: Array<TimeDisplayConfig['format']> = ['current/total', 'current', 'remaining'];
  ...
  protected onStateUpdate(state: PlayerState): void {
    ...
    if (this.currentTimeElement) {
      if (config.format === 'remaining') {
        ...
        this.currentTimeElement.textContent = this.formatTime(state.currentTime, config.showMilliseconds);
      }
    }

    // 更新总时间
    if (this.totalTimeElement) {
      this.totalTimeElement.textContent = this.formatTime(state.duration, config.showMilliseconds);
    }
  }
  ...
  private formatTime(seconds: number, showMilliseconds = false): string {
    ...
    if (hours > 0) {
      ...
      return showMilliseconds ? `${timeStr}.${ms.toString().padStart(3, '0')}` : timeStr;
    } else {
